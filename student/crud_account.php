...
if (isset($_SESSION['id']) && isset($_SESSION['username']) && isset($_SESSION['userType']) && 
   ($_SESSION['userType'] === 'student' || $_SESSION['userType'] === 'parent')) {
    ...
    if (!empty($new_email)) {
        ...
        if ($email_result->num_rows > 0) {
            ...
            if ($conn->query($update_query) === TRUE) {
                ...
                // Error message
                $_SESSION['message_danger'] = "Failed to update account information.";
                header('Location: student_dashboard.php');
                exit();
            }
        }
        ...
        $update_query = "UPDATE users SET email = NULL WHERE id = $user_id";
        if ($conn->query($update_query) === TRUE) {
             $_SESSION['message'] = "Account information updated successfully.";
            header('Location: student_dashboard.php');
            exit();
    ...
    if ($result->num_rows == 1) { // Check if current password is correct
      if (preg_match('/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&_#^+=(){}[\]|\\:;"\'<>,.~`]).{8,}$/',$new_password)) {
        ...
        if ($new_password === $confirm_password) {
          ...
          header("Location: ../index.php?success=Password updated successfully! Please log in with your new password.");
          exit(0);
        } else {
